...
def get_processed_data(datasetFile, categoryMappingsPath, classType='binary'):
    ...
    category_1 = np.array(pd.read_csv(categoryMappingsPath + "1.txt", header=None).iloc[:, 0].values) # change .csv ot .txt
    ...
    X = np.array(ct.fit_transform(X), dtype=float) # change np.float--->float
